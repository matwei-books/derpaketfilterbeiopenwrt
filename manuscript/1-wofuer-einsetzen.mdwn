...
Wofür kann ich OpenWrt einsetzen?
=================================

Nachdem ich nun geklärt habe, was OpenWrt ist, ist meine nächste Frage:
wofür ich das einsetzen kann.
Dazu kann ich hier nur einen kurzen Ausschnitt geben, ich beschränke mich
...
Das sind nur Anwendungsfälle, bei denen ich den Paketfilter auf jeden Fall
nutzen will.

Internet-Zugangsrouter
----------------------

Hierbei nutze ich meist einfache, stromsparende Hardware, wie zum Beispiel
einen billigen SOHO-Router, auf dem ich OpenWrt installiert habe.
Die Liste der unterstützten Geräte finde ich unter
<http://wiki.openwrt.org/toh/start>.

Hier muss ich bei IPv4 meistens Masquerading zum Uplink einstellen, um meine
internen Adressen zu verbergen.
Will ich dann Dienste nach außen anbieten oder bestimmte Spiele spielen,
welche Verbindungen vom Server zum Client benötigen, muss ich auch
das Weiterleiten bestimmter Ports konfigurieren.
...
meiner Absenderadresse umgehen will.
...
beschränken will oder Masquerading gegenüber dem anderen Tunnel-Endpunkt
einsetzen will.
...
habe.
Möglicherweise kann ich hier auch OpenWrt in einer VM verwenden.
...
Den Tunnel-Endpunkt kann ich wieder beim Internet-Zugangsrouter einrichten
oder in einem separaten System, welches in meinem Netz dann als IPv6-Router
fungiert.
Dieses benötigt dann ebenfalls nur einen Netzwerkadapter.
...
Hardware aus.
Der Paketfilter hilft mir hier den Datenverkehr in geordnete Bahnen zu
lenken.
